   ...
   public class ExportPerformanceDAL
    {
       ...
       public static DataTable GetExportPerformanceWithoutBL(int LineID, int LocationID, DateTime StartDate, DateTime EndDate, int ServiceID, string Stat, int VesselID, long VoyageID)
       {
           string strExecution = "[exp].[rptExportPerformanceWithoutBL]";
           DataTable dt = new DataTable();
           using (DbQuery oDq = new DbQuery(strExecution))
           {
               oDq.AddIntegerParam("@LineID", LineID);
