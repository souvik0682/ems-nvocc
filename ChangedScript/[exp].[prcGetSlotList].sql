...
/****** Object:  StoredProcedure [exp].[prcGetSlotList]    Script Date: 05/01/2013 18:19:05 ******/
SET ANSI_NULLS ON
GO
...
BEGIN  
 ...
 AND  ((ISNULL(@SchPODName, '') ='') OR (exp.GetPortName(slot.fk_POD,'B') LIKE '%'+@SchPODName + '%'))  
 AND  ((ISNULL(@SchPOLName, '') ='') OR (exp.GetPortName(slot.fk_POL,'B') LIKE '%'+@SchPOLName + '%'))  
 AND		((ISNULL(@SchEffDate, '') ='') OR (EffDate between convert(datetime,@SchEffDate ) and convert(datetime,@SchEffDate )))
